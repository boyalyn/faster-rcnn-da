...
class FasterRCNNTrainer(nn.Module):
    ...
    * :obj:`total_loss`: The sum of 4 loss above.
    ...
    def forward(self, imgs, bboxes, labels, scale, domain_label):
        # print(domain_label)
        ...
        * :math:`R` is the number of bounding boxes per image.
        ...
        da_ins_loss, da_img_loss = self.faster_rcnn.da_forward(features, rois, roi_indices, domain_label)
        losses = [da_ins_loss, da_img_loss]
        
        if domain_label == "target":
            # print("entering target domain...")
            losses += [torch.tensor(0)]*4
            losses += [sum(losses)+10*losses[1]]
        ...
        sample_roi_index = t.zeros(len(sample_roi)).to(opt.device)
        sample_roi = t.from_numpy(sample_roi).to(opt.device)
        ...
        gt_rpn_loc, gt_rpn_label = self.anchor_target_creator(
            ...
            anchor,
            img_size)
        gt_rpn_label = at.totensor(gt_rpn_label).long().to(opt.device)
        gt_rpn_loc = at.totensor(gt_rpn_loc).to(opt.device)
        ...
        roi_cls_loss = nn.CrossEntropyLoss()(roi_score, gt_roi_label.to(opt.device))
    ...
    def train_step(self, imgs, bboxes, labels, scale, domain_label):
        ...
        losses = self.forward(imgs, bboxes, labels, scale, domain_label)
...
def _fast_rcnn_loc_loss(pred_loc, gt_loc, gt_label, sigma):
    in_weight = t.zeros(gt_loc.shape).to(opt.device)
